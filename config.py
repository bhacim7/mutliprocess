# ==========================================
# CONFIGURATION FILE
# ==========================================

# --- SYSTEM SETTINGS ---
NAV_MODE = "GPS"
STREAM = True
GCS_IP = "192.168.1.105"
RECORD_VIDEO = False
SHOW_LOCAL_WINDOW = False

# --- HARDWARE SPECS ---
ROBOT_RADIUS_M = 0.3
INFLATION_MARGIN_M = 0.6 # Increased to enforce a wider berth around obstacles
MAX_TILT_ANGLE = 5.0

# --- PINS / CHANNELS ---
SOL_MOTOR = 6
SAG_MOTOR = 3
FRONT_SOL_MOTOR = 5
FRONT_SAG_MOTOR = 2
FRONT_STEER_SERVO = 7

BASE_PWM = 1500
CRUISE_PWM = 130
T3_SPEED_PWM = 100

STEER_MAX_PWM = 1900
STEER_MIN_PWM = 1100
STEER_DEADBAND_DEG = 5.0

# --- SENSORS ---
SERIAL_PORT = "/dev/ttyUSB0"
SERIAL_BAUD = 57600
HEADING_SOURCE = 'ZED'

FC_PORT = "/dev/ttyACM0"  # OrangeCube Uçuş Kontrolcüsü Portu
FC_BAUD = 57600

LIDAR_PORT_NAME = "/dev/ttyUSB1"
LIDAR_BAUDRATE = 1000000
LIDAR_MAX_DIST = 10.0
LIDAR_ACIL_DURMA_M = 1.5
LIDAR_FREE_GAIN = 25
ENABLE_LIDAR = False
LIDAR_OCCUPIED_GAIN = 80
LIDAR_KORIDOR_KP = 30.0
MAP_DECAY_AMOUNT = 1

CAM_RES = 2
CAM_FPS = 30
CAM_HFOV = 110.0
YOLO_CONFIDENCE = 0.50
MODEL_PATH = "/home/arge/PycharmProjects/PythonProject/best.engine"
Kp_PIXEL = 0.3
Kd_PIXEL = 0.1

# --- NAVIGATION CONTROL ---
SPOT_TURN_THRESHOLD = 35.0
SPOT_TURN_PWM = 200
ESCAPE_PWM = 300
MAX_PWM_CHANGE = 60
HYBRID_STEP_DIST = 2.0
HYBRID_HEADING_THRESHOLD = 30.0
ENABLE_LOS_GUIDANCE = True
LOS_KP = 1.0

# --- DIRECT DRIVE PID ---
DIRECT_DRIVE_KP = 1.2
DIRECT_DRIVE_KI = 0.05
DIRECT_DRIVE_KD = 0.8
ANTI_WINDUP_DEG = 15.0

# --- PURE PURSUIT (A* PATH FOLLOWING) ---
PURE_PURSUIT_MIN_LOOKAHEAD = 2.0
PURE_PURSUIT_MAX_LOOKAHEAD = 2.5
PURE_PURSUIT_K_SPEED = 0.5
PURE_PURSUIT_KP = 3.0
PURE_PURSUIT_KD = 0.8
A_STAR_HEURISTIC_WEIGHT = 2.5
COSTMAP_RES_M_PER_PX = 0.10

# --- TASK WAYPOINTS (Legacy values from IDA1) ---
T1_GATE_ENTER_LAT = 40.8090506
T1_GATE_ENTER_LON = 29.2621206
T1_GATE_MID_LAT = 40.8090633
T1_GATE_MID_LON = 29.2622039
T1_GATE_EXIT_LAT = 40.8092299
T1_GATE_EXIT_LON = 29.2621285

T2_ZONE_ENTRY_LAT = 40.8092299
T2_ZONE_ENTRY_LON = 29.2621285
T2_ZONE_MID_LAT = 40.8093862
T2_ZONE_MID_LON = 29.2621295
T2_ZONE_END_LAT = 40.809563
T2_ZONE_END_LON = 29.2622042

ENABLE_TASK3 = True
T3_START_LAT = 40.8095884
T3_START_LON = 29.2622185
T3_MID_LAT = 40.8096174
T3_MID_LON = 29.2622489

MEVCUT_GOREV = "TASK2_START"
